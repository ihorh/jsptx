...
/* Where jsp_run reads and writes. trace_fd is meaningful only when options
   below sets trace to something other than JSP_TRACE_NONE. */
typedef struct {
    int in_fd;
    int out_fd;
    int trace_fd;
...
/* What jsp_run does once it's reading. */
typedef struct {
    size_t          buf_size;
    jsp_output_mode mode;
...
/* Reads fds.in_fd until end of file, classifying the structural JSON
   ...
   options.mode decides what reaches fds.out_fd. JSP_OUTPUT_SINK writes
   ...
   options.trace decides what reaches fds.trace_fd, the same way and at
   the same time, whatever mode is doing with out_fd. JSP_TRACE_NONE writes
   nothing. JSP_TRACE_OFFSETS writes one line per structural character
   found: "<offset>\t<char>\n", offset absolute in the input stream.
   JSP_TRACE_MASKS writes one line per block instead: "<offset>\t<mask>\n",
   mask the block's 64-bit classification as 16 lowercase hex digits and
   offset the block's first byte.

   Rounds options.buf_size up to the nearest multiple of 64, with a minimum
   of 64, and reads in chunks of that size. Returns 0 on success, -1 on a
   read or write error with errno set by the failing call, or, with
   JSP_OUTPUT_PLUCK, on malformed input: unbalanced or mismatched brackets,
   or nesting past JSP_MAX_DEPTH. */
